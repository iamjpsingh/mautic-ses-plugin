...
class SegmentFilterSubscriber implements EventSubscriberInterface
{
    ...
    public static function getSubscribedEvents(): array
    {
        ...
        // Register filtering handler if the event exists in this Mautic version
        if (defined('Mautic\\LeadBundle\\LeadEvents::LIST_FILTERS_ON_FILTERING')) {
    ...
    public function onGenerateSegmentFilters(LeadListFiltersChoicesEvent $event): void
    {
        $event->addChoice('lead', 'ses_bounced', [
            'label'      => $this->translator->trans('plugin.amazonses.segment.ses_bounced'),
        ...
        $event->addChoice('lead', 'ses_complained', [
            'label'      => $this->translator->trans('plugin.amazonses.segment.ses_complained'),
            'properties' => ['type' => 'boolean'],
            'operators'  => ['include' => ['eq']],
            'object'     => 'lead',
        ]);
     ...
     * This method is only called if Mautic dispatches the LIST_FILTERS_ON_FILTERING event.
     */
